<template>
  <div class="proposal-list">
      ...
      <mt-button @click="go('/home')" slot="left">
        <i class="fa fa-angle-left"></i>
      </mt-button>
    </mt-header>
    <div class="toolbar" >
    	<div @click="goto">
				<img src="../../../static/img/manage/add.png" height="17" width="18">
				<span>新增</span>
			</div>
			<div @click="search">
      ...
      <div class="manage-main-content_1">
        ...
        <ul>
          ...
          <li class="manage-data">
            ...
            <date-picker v-model="form.startDay" @change="test"/>
            <p class="manage-data-to">至</p>
            <date-picker v-model="form.endDay" @change="test"/>
          </li>
          <li class="manage-pdf">
...
export default {
      ...
      if(this.dateSub > 0){
        ...
        utils.http.post('PROPOSALLIST',requ).then(response => {
          ...
          if(this.proposalList.length <= 0){
            utils.ui.modal.afterOpen()
            this.$refs.showalert.showMOdal('没有查询到相关数据！')
            // MessageBox({
            //   title: '提示',
            //   message: '没有查询到相关数据！'
            // }).then(()=>{
  ...
  mounted(){
    this.products = productItem.relation.item
  }
}
</script>
