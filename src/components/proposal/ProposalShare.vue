      ...
      <div>
        ...
        <div class="slide-wraper">
          ...
          <div class="button-prev" id="btn1">
            <i @click="chooseImg" class="fa fa-angle-left"></i>
          </div>
  ...
  export default{
    ...
    data(){
      return{
        ...
        banners:[{
          imgName:'120.jpg',
          src:require('../../../static/img/enjoy/120.jpg')
        },{
          imgName:'130.jpg',
    ...
    methods:{
      getDemoData() {
        utils.http.post('PROPOSALHQMSG', {
          ...
          console.log('获取生成建议书需要的数据',)
          let productList =  response.data.response.proposalPlan.pdfMsgDTO.productList
          for(let i=0;i<productList.length;i++){
            if(productList[i].ismain == "1" && productList[i].productCode == "NPT011"){
                this.productCode = productList[i].productCode 
            }
          }
          this.shareMsg.agentCode = response.data.response.proposalPlan.pdfMsgDTO.zyzh
          this.shareMsg.agentName = response.data.response.proposalPlan.pdfMsgDTO.sxgw
          this.shareMsg.agentMobile = response.data.response.proposalPlan.pdfMsgDTO.phone
        })
      },
      ...
      makeMsgData:function(){
		...
		this.$refs.showalert.showMOdal(project.proposalPlan.resultMessage)
            //MessageBox({
             // title: '提示',
              //message: project.proposalPlan.resultMessage
            //}).then(()=>{
             // utils.ui.modal.beforeClose()
            //})
          }
        ...
        Wechat.share({
            message: {
                ...
                media: {
                    ...
                    webpageUrl: "http://"+url+"/com.ifp.ipartner/proposalShare/index.html#/proposal/demo/"+this.planId
                }
            },
