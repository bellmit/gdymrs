...
<script>
	...
	export default{
        ...
        mounted(){
            ...
            this.DLSelfHoldAllInfo.productName=productNameList.slice(0,length-1)
            //缴费*元
	    	...
	    	getDlSelfHoldEntryAll(){
	    	    ...
	    	    utils.http.post('MOHQPRODUCTMSG',requ).then(response => {
	    	        if(response.data.header.responseCode == "0"){
	    	        	...
	    	        	this.DLSelfHoldAllInfo.insuranceLiability = insuranceList
	    	           console.log("保险责任",this.DLSelfHoldAllInfo.insuranceLiability)
	    	           //免除责任
	    	           ...
	    	           this.DLSelfHoldAllInfo.exemptionFromLiability = exemptionFromList
	    	           console.log("免除责任",this.DLSelfHoldAllInfo.exemptionFromLiability)
	    	        }
	    	    }).catch(error => {
