					...
					<div class="table">
						<table>
							...
							<tr>
								...
								<td style="width:24%;font-size:10px"><img :src="sign" alt="" style="width:30px;height:30px;" ></td>
							</tr>
							<tr>							
								<td colspan="4" style="text-align:left;">
									&nbsp;&nbsp;1、<b>投保人所交钱款是用于购买保险<input type="text" readonly style="    border-bottom: 1px solid #000;" v-model="TJPersonalInfo.mngShortName">公司的保险产品，不是购买银行理财产品，也不是办理银行存款赠送的保险，银行不承担保本保息的责任，投保人提前退保可能承担一定损失。</b>
								</td>		
							</tr>
							<tr>	
								<td colspan="4" style="text-align:left;">
							...
							<tr>
								<td colspan="4" style="text-align:left;">
									&nbsp;&nbsp;5、投保人签收保险单后有<b class="btext">10天犹豫期</b>。犹豫期内投保人可以无条件退保，保险公司扣除不超10元的成本费后退还全部保费。<b class="btext">超过犹豫期后退保，投保人将承担一定的退保损失</b>，具体损失金额可查阅保险单所附“现金价值表”或询问保险公司。<br/>
								</td>
							</tr>
							<tr>
								<td colspan="4" style="text-align:left;">
							...
							<tr>
								<td colspan="4" style="text-align:left;">
									&nbsp;&nbsp;销售人员声明：我已向投保人逐条宣讲以上内容。                签字：<img :src="signImgsAgent" alt="" style="border-bottom:1px solid #000;width:30px;height:30px;"> 日期：
								</td>
							</tr>
							<tr>
								<td colspan="4" style="text-align:left;">
									&nbsp;&nbsp;投保人声明：该销售人员已向我逐条宣讲以上内容。        签字：<img :src="sign" alt="" style="border-bottom:1px solid #000;width:30px;height:30px;">日期：
								</td>
							</tr>
						</table>
					</div>			
...
<script>
	...
	export default{
	    ...
	    data() {
	        return {
	            ...
	            TJPersonalInfo:{
	            	...
	            	allPayPrem1:"",
	            	otherPayIntvDes:"",
	            	mngShortName1:"光大永明人寿保险有限公司",
	            	mngShortName2:"光大永明人寿保险有限公司",
	            	comPhone:"95348",
	            	typeProduct:"",	
	            	riskTypeItem:"",
        ...
        mounted(){
            $(".btnFoote").css({'opacity':0.5})
            //代理人名字
            this.TJPersonalInfo.agentName=this.$route.params.agentName
            this.TJPersonalInfo.typeProduct=this.$route.params.typeProduct
            ...
            console.log("数据",productsList)
            // 代理人机构名称
            this.TJPersonalInfo. agentComName=productsList.agentInfo.managename
            let products=productsList.productInfo.products.product
            console.log("险种",products)
            //保费和
           	let premSum=productsList.productInfo.sumPremL
           	console.log("保费和",premSum)
            for(let i=0;i<products.length;i++){
            	if(products[i].isMaster=="1"){
            		this.TJPersonalInfo.riskName = products[i].prodName
            		console.log("主险种产品",products[i].prodName)
            		if (products[i].payIntv=="single") {
            			...
            			this.TJPersonalInfo.payYears=this.getPay(products[i].payYear)
            			this.TJPersonalInfo.prem=premSum
            			this.TJPersonalInfo.allPayPrem=((premSum)*this.getPay(products[i].payYear)).toFixed(2)
            		}
            	}
            }
        },
