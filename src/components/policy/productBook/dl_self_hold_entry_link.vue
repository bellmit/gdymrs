...
<script>
	...
	export default{
        ...
        mounted(){
            ...
            this.DLSelfHoldLinkInfo.productName=productNameList.slice(0,length-1)
            //缴费*元
	    	...
	    	getDlSelfHoldEntryLink(){
	    	    ...
	    	    utils.http.post('MOHQPRODUCTMSG',requ).then(response => {
	    	        if(response.data.header.responseCode == "0"){
	    	        	...
	    	        	this.DLSelfHoldLinkInfo.insuranceLiability = insuranceList
	    	           console.log("保险责任",this.DLSelfHoldLinkInfo.insuranceLiability)
	    	           //免除责任
	    	           ...
	    	           this.DLSelfHoldLinkInfo.exemptionFromLiability = exemptionFromList
	    	           console.log("免除责任",this.DLSelfHoldLinkInfo.exemptionFromLiability)
	    	        }
	    	    }).catch(error => {
