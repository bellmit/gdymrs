...
<template>
	<div id="content">
		<Headerbtns :title = "page_title">
			<div slot="left" class="back" @click="go('../inform/'+$route.params.planId)">
				...
				<div class="inform_1">
					...
					<div class="textarea" v-show="itm.swh && itm.code!='A00001'" >
						<p style="display:flex;" v-show="itm.swh && itm.code=='A00013'">
							<span>&nbsp;&nbsp;&nbsp;&nbsp;怀孕日期:&nbsp;&nbsp;&nbsp;&nbsp;</span>
							<date-picker v-model="policy.question.resQuestion.fetationDateAppnt"></date-picker>
						</p>
...
export default{
	...
	components: {Headerbtns,Footerbtns,Actionsheet,alert,datePicker},
	data () {
    return {
		sheetVisible:false,
		actions:[
  ...
  methods:{
	 left(){
         this.go('../inform/'+this.$route.params.planId)
	  },
	  right(){
		 this.next()
	  },
	  ...
	  getPolicyMsg(){
		...
		utils.http.postFast('MOHQPOLICYMSG',requ,(body)=> {
			...
			//提取客户信息
		    console.log(policy)
			this.temp.insured.gender = policy.custInfo.appnt.appntSex
			this.temp.insured.birthday = policy.custInfo.appnt.appntBirthday
			//加载代理人信息
			...
			if (this.policy.agent!=null) {
				...
				for (var i = 0; i < this.dict.informList.length; i++) {
					if(this.temp.cusType == 0){
						this.dict.informList[i].resCustCode = "0"
					}
				}
				let tempResRemark = []
				if(policy.question.resQuestion.resRemark != undefined && policy.question.resQuestion.resRemark.length > 0){
					let resRemark = policy.question.resQuestion.resRemark
						console.log(resRemark )
					for(var i=0;i<resRemark.length;i++){
						if(resRemark[i].resCustCode == "0"){
							tempResRemark.push(resRemark[i])
						}else if(resRemark[i].resCustCode == "1"){
						...
						for (var j = 0; j < policy.question.resQuestion.resRemark.length; j++) {
							...
							if(this.dict.informList[i].code == resRemark.code){
								if(this.dict.informList[i].code == "A00001"){
									...
									console.log(this.dict.informList[i].weight)
								}
	...
	productDef:function(callback){
  	...
  	toSalesmanReportPage:function(){
  		...
  		if (isPass) {
  			this.$router.push({
	  			path:'/policy/salesman_report_page.vue',
  	...
  	switchQuestion:function(itm){
  		if (itm.isBuild == "Y") {
			...
			itm.isBuild = "N"
			if(itm.code == 'A00013'){
				this.policy.question.resQuestion.fetationDateAppnt=""
			}
	...
	this.productDef((productDefList)=>{
		if(productDefList[0].productDefine.hasHealthTold == true 
			...
			for(var i=0;i<extraList.length;i++){
				if(extraList[0].productDefine.hasHealthTold == true 
					|| extraList[0].productDefine.hasHealthTold == "true"){
						this.isHaveAppInform = true
						break
					}
			}
...
.manage-main-contents{
	...
	.inform_1 .inform-content p:nth-child(1){border-radius: 20px;text-align: center;color: #000;}
