	...
	<div id="content" >
		...
		<div v-show="contentIsOpen">
		    <Headerbtns :title="'电子投保单'">
		    	<div slot="left" class="back" @click="backSales">
			...
			<div class="manage-main-contents">
					...
					<div class="appnt">
						...
						<div class="applicantUser" v-show="toggle.isAppIdInform">
							<div style="padding:0 5%;" class="inform-content">
								<div style="width:50%">
									<p class="user">姓名:&nbsp;{{applicant.name}}</p>
									<p class="user">婚姻:&nbsp;{{applicant.marriage}}</p>
									<p class="user">证件类型:&nbsp;{{applicant.idType}}</p>
								</div>
								<div style="width:50%">
									<p class="user">国籍:&nbsp;{{applicant.country}}</p>
									<p class="user">性别:&nbsp;{{applicant.gender}}</p>
									<p class="user">出生日期:&nbsp;{{applicant.birth}}</p>
								</div>
							</div>
							<div style="padding:0 5%;">
								<p>证件号码:{{applicant.idNumber}}</p>
								<p class="user">证件有效期:&nbsp;{{applicant.idTime}}</p>
								<p class="user">年收入:&nbsp;{{applicant.yearIncome}}万元</p>
							</div>
						</div>
					</div>
				</div>
				<div class="border-radius">
					<div class="appnt">
						...
						<div class="applicantUser" v-show="toggle.isAppContactInform">
							<div class="line">
								<p>联系地址:&nbsp;{{applicant.province}}{{applicant.city}}{{applicant.county}}{{applicant.street}}{{applicant.community}}{{applicant.postalAddress}}</p>
							</div>
							<div class="line">
								<p class="user">邮政编码:&nbsp;{{applicant.emsCode}}</p>
								<p class="user">联系电话:&nbsp;{{applicant.mobileTel}}</p>
								<p class="user"></p>
							</div>
							<div class="line">
				...
				<!-- 保险业务人员声明 -->
				<div class="border-radius" style="margin-bottom:20%;">
					<div class="appnt">
						...
						<div v-show="dataShow.dataShow11" v-model="dataShow.dataShow11" class="insureAccountInfo">
							...
							<div class="infoSign3">
								<p @click="openCADialog(23,$event)">
									<img style="width:100%;height:100%;"
									id="xss_23" :src="wrting">
								</p>
								<p @click="openCADialog(24,$event)">
									<img style="width:100%;height:100%;" id="xss_24" :src="wrting">
								</p>
								<p @click="openCADialog(25,$event)">
							...
							<div class="infoBox">
								...
								<p>
									<span>投保人已阅读并确认</span>
									<span>
										<b class="specialDef" v-if="tmp.isRead" style="color:#0C3752" v-for="tmp of definesLists" @click="jumpToTeShuDanZheng(tmp.fileName)">{{"《"+tmp.name+"》"}}</b>
		  ...
		  left(){
	       ...
	       right(){
			...
			shareApp:function(){
				...
				let shareLink = "http://" + host + "/com.ifp.ipartner/proposalShare/index.html#/policy/electronics_share/"+this.$route.params.planId+"/"+this.$route.params.applyCode+"/24/-1/-1"
			...
			shareIns:function(){
				...
				let host = window.globalConfig.rootUrl.split('/')[2]
				let shareLink = "http://" + host + "/com.ifp.ipartner/proposalShare/index.html#/policy/electronics_share/"+this.$route.params.planId+"/"+this.$route.params.applyCode+"/25/-1/-1"
				console.log("分享地址 ===>" + shareLink)
				Wechat.share({
					message: {
						title: "电子投保单",
			...
			this.offsetHeight = event.currentTarget.offsetTop
				...
				if(signNum == "23"){
					//业务人员
					cust = {
						name:this.tempPolicy.agentInfo.name,
						idNum:this.tempPolicy.agentInfo.idno,
						idType:"11"
					}
					this.openCA(23)
					return
				...
				if(signNum == "24"){
					//投保人
					cust = {
						name:this.tempPolicy.custInfo.appnt.appntName,
						idNum:this.tempPolicy.custInfo.appnt.idNo,
						idType:this.tempPolicy.custInfo.appnt.idType
					}
				}
				if(signNum == "25"){
					//被保人
					let age = this.getAge(this.tempPolicy.custInfo.insurants.insurant[0].birthday)
					if(age < 18){
						cust = {
							name:this.tempPolicy.custInfo.appnt.appntName,
							idNum:this.tempPolicy.custInfo.appnt.idNo,
						...
						cust = {
							name:this.tempPolicy.custInfo.insurants.insurant[0].name,
							idNum:this.tempPolicy.custInfo.insurants.insurant[0].idNo,
							idType:this.tempPolicy.custInfo.insurants.insurant[0].idType
						}
					}
				}
				console.log("进行识别信息=====>"+signNum)
				console.log(cust)
...
<style scoped type="text/css" lang=scss>
	.tip{width:25px;height: 25px;border-radius: 50%;background: #FEB101;color: #fff;display:inline-block;
