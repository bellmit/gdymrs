	...
	<div style="background-color:#F5F5F5;font-size:12px">
	    ...
	    <div class="manage-main-contents">
	    	<div style="text-align:right;padding:8px 15px 8px 0;position:relative;overflow-y:hidden">
	    		<button id="d1" class="btn color status-p"  @click="ProductList(1)" style="color:#ffff">未完成</button>
	    		...
	    		<button id="d2" class="btn color1 txtColor status-p" @click="ProductList(2)" style="color:#ffff">已完成</button>
	    	...
	    	<ul class="mar-btm"id="ul1" v-if="unSucList"   v-infinite-scroll="loadMore"
	    	  ...
	    	  infinite-scroll-distance="10">
	    		...
	    		<li v-if="orderlist1.item.length == 0">
	    			<div style="line-height:100px;text-align:center">
	    				当前列表为空!
	    			</div>
	    		</li>
	    		<li class="ul-li" v-for="(item,index) in orderlist1.item" :key="index">
	    			<ul>
	    				<li v-for="(item,index) in item.unsuccOrder" :key="index">
			    			<div class="f-flex lh">
				    			...
				    			<div>
					    			<button @click="statusChange(item)" class="clear btn status-btn"  v-if="item.status != -1 && item.status != 6">
			    			...
			    			<div style="line-height:25px;">
			    				...
			    				<button v-if="item.status == 80 || item.status == 81 || item.status == 82 || item.status == 84" class="btn bgcolor txtColor rt-ft padd"  @click="goFillout(item)">继续填写</button>
			    				...
			    				<button class="btn bgcolor txtColor rt-ft padd"  @click="getStatausDetail(item)" v-if="item.status != 80 && item.status != 81 && item.status != 82 && item.status != 83 && item.status != 84  && item.status != -1 && item.status != 5 &&  item.status != 7 ">查看</button>
	    		...
	    		<li v-show="allSize1 == allLength && allLength > 3">
...
<script>
	...
	// import policyList from "../../assets/data/policy_name.json"
	export default {
	  name: 'backlog',
	  ...
	  methods:{
	  	loadMore() { 
	  	    if (this.index === 1) {
	  	    	...
	  	    	if(this.array1.length == 10){
	  	    		this.getOrderList(2,++this.number1);
	  	    		console.log(this.succOrder.length)
	  	    		console.log(this.allSize2)
		  	    	return
	  	    	}
	  	    }
	  	},
	  	...
	  	goDelete(item,index){
			...
			utils.http.postFast('MODELPOLICY',requestParam,(body)=>{
				...
				if(body.policy.resultCode == 0){
					...
					let code = null;
					this.orderlist1.item.forEach(elem=>{
						code = elem.code
	  	...
	  	// 			console.log('获取查询列表',this.allSize1)
	  	// 			console.log("length11111111",this.unsuccOrder.length)
  		...
  		// 			console.log('获取查询列表',this.allSize2)
	  	// 			console.log('获取查询列表',this.succOrder.length)
	  	...
	  	getOrderList(num,value){
  			...
  			utils.http.post('MOPOLICYLIST',requ).then(response => {
  				if(num == 1){
					...
					this.orderlist1.item.push({code:value,
							unsuccOrder:this.array})
					let length = null
					this.orderlist1.item.forEach(elem=>{
						length += elem.unsuccOrder.length
						console.log(elem,'elem')
						elem.unsuccOrder.forEach(element=>{
					...
					console.log(this.orderlist1.item,'========')
					// this.unsuccOrder = this.unsuccOrder.concat(this.array);
					// console.log(this.array.length,'返回列表')
					// this.orderId = this.unsuccOrder;
					this.getStatusAll();
  				}else if(num == 2){
				 //  	this.allSize2 = response.data.response.allSize
				 //  	this.array1 = response.data.response.order
				 //  	console.log(this.array1.length,'已完成=========')
	  	...
	  	ProductList(num){
	  		...
	  		if(num == 1){
		  		...
		  		// if(this.unsuccOrder.length == ""){
		  		// 	this.$refs.showalert.showMOdal('未完成列表为空!')
		  		// }
		  		...
		  		// if(this.succOrder.length == ""){
		  		// 	this.$refs.showalert.showMOdal('已完成列表为空!')
		  		// }
		  		// else{
		  			// this.succOrder = null;
		  			// this.getOrderList(2,this.number1);
				...
				if(products!=undefined && products.length>0){
            		for (var i = 0; i < products.length; i++) {
					  ...
					  if(products[i].isMaster == 1 && products[i].prodCode == "LBP017"){
						  ...
						  // 鑫玺人生
					  if(products[i].isMaster == 1 && products[i].prodCode == "NPP016"){
							mainProduct.dividend = products[i].bonusGetMode
						}
					  //特殊处理增利宝追加保费和部分领取
        		...
        		if(orgId.substring(0,4) == '8661'){//重庆机构
			        ...
			        utils.http.postFast('MOORDERDEAL',requ,(body)=> {
		          		...
		          		if(policy.resultCode == "0"){
		          			...
		          			this.$refs.showalert.showMOdal(policy.applyCode + "\n" +policy.resultMessage)
		          			setTimeout(()=>{
		          				this.$router.push({name:'product_details_page',params:{planId:item.orderId}})
		          			},1000)
		          		}
		        	})
            		// this.$router.push({name:'salesman_report_page',params:{planId:item.orderId}})
            	}
            })
	  	},
	  	//所有保单状态更新
	  // 	getStatusAll(){
			// let arr = []
	  // 		this.unsuccOrder.forEach(element=>{
			...
			// 	for(var i=0;i<this.unsuccOrder.length;i++){
			// 		let applyCode = this.unsuccOrder[i].applyCode
			// 		if(applyCode!=""){
			// 			for(var j=0;j<orderList.length;j++){
			// 				if(orderList[j].prtNo == applyCode){
			// 					this.unsuccOrder[i].status = orderList[j].state
			// 					console.log("修改为最新状态")
			// 				}
			// 			}
			// 		}
